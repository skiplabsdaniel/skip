...
export class ServiceInstance {
  ...
  reload(ctors: (new (...params: DepSafe[]) => object)[]) {
    ...
    for (const ctor of ctors) {
      ...
      for (const [handle, collection] of links.entries()) {
        ...
        const info = this.refs.handles.get<unknown>(handle as Handle<unknown>);
        if (
          info &&
          typeof info == "object" &&
          "object" in info &&
    ...
    if (collections.size > 0) {
      const errorHdl = this.refs.runWithGC(() => {
        ...
        return this.refs.binding.SkipRuntime_invalidate(skcollections);
      });
      if (errorHdl) throw this.refs.handles.deleteHandle(errorHdl);
    }
  }
