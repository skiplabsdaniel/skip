...
void SkipRuntime_VoidExecutor__resolve(uint32_t checkerId) {
  Isolate* isolate = Isolate::GetCurrent();
  Local<Object> externFunctions = kExternFunctions.Get(isolate);
  Local<Value> argv[1] = {Number::New(isolate, checkerId)};
  CallJSVoidFunction(isolate, externFunctions,
                     "SkipRuntime_VoidExecutor__resolve", 1, argv);
}

void SkipRuntime_VoidExecutor__reject(uint32_t checkerId, double handle) {
  Isolate* isolate = Isolate::GetCurrent();
  Local<Object> externFunctions = kExternFunctions.Get(isolate);
  Local<Value> argv[2] = {Number::New(isolate, checkerId),
                          Number::New(isolate, handle)};
  CallJSVoidFunction(isolate, externFunctions,
                     "SkipRuntime_VoidExecutor__reject", 2, argv);
}

void SkipRuntime_deleteVoidExecutor(uint32_t checkerId) {
  Isolate* isolate = Isolate::GetCurrent();
  Local<Object> externFunctions = kExternFunctions.Get(isolate);
  Local<Value> argv[1] = {Number::New(isolate, checkerId)};
  CallJSVoidFunction(isolate, externFunctions, "SkipRuntime_deleteVoidExecutor",
                     1, argv);
}
