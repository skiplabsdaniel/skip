...
double SkipRuntime_invalidate(CJArray collections);
...
void Invalidate(const FunctionCallbackInfo<Value>& args) {
  Isolate* isolate = args.GetIsolate();
  if (args.Length() != 1) {
    // Throw an Error that is passed back to JavaScript
    isolate->ThrowException(
  ...
  NatTryCatch(isolate, [&args](Isolate* isolate) {
    ...
    double skresult = SkipRuntime_invalidate(skcollections);
...
void GetToJSBinding(const FunctionCallbackInfo<Value>& args) {
  ...
  AddFunction(isolate, binding, "SkipRuntime_invalidate", Invalidate);
  //
